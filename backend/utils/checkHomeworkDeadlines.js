...
const checkHomeworkDeadlines = async () => {
  try {
    ...
    for (const lesson of lessonsWithHomework) {
      ...
      const deadline = new Date(lesson.createdAt.getTime() + lesson.homeworkDeadline * 60 * 60 * 1000) // Добавляем часы
      ...
      for (const enrollment of enrollments) {
        ...
        if (!submission && new Date() > deadline) {
          console.log(`Student ${enrollment.user} did not submit homework for lesson ${lesson._id}`)

          // 2.3.3. Списываем жизнь
          enrollment.lives -= 1
          await enrollment.save()

          console.log(`Life deducted from student ${enrollment.user}. Remaining lives: ${enrollment.lives}`)

          // 2.3.4. Если у студента не осталось жизней, закрываем ему доступ к курсу
          if (enrollment.lives <= 0) {
            console.log(`Student ${enrollment.user} has no more lives. Access to the course is closed.`)

            // Закрываем доступ к курсу
            enrollment.isLocked = true // Используем isLocked
            await enrollment.save()

            console.log(`Access to course ${lesson.course} closed for student ${enrollment.user}`)
          }
        }
      }
    }
